...
var Model = function () {
    ...
    var calcCard = function calcCard() {
        var randNum = [0, 'A', 2, 3, 4, 5, 6, 7, 8, 9, 'J', 'Q', 'K'];
        var randState = ['C', 'D', 'H', 'S', 'C', 'D', 'H', 'S', 'C', 'D', 'H', 'S'];
        var arr = [];
        var finallArr = [];
    ...
    var calcPoints = function calcPoints(cards, state) {
        if (state) {
    ...
    var checkPairs = function checkPairs(card, cards) {
    ...
    var checkCards = function checkCards(cardsNumber, todoo) {
        var cardsNum = cardsNumber;
        var todo = todoo;

        var points = calcPoints(cardsNum, todo);
...
var Ui = function () {
    ...
    var startGame = function startGame(item) {
    ...
    var endGame = function endGame(item) {
    ...
    var startNewGame = function startNewGame(e) {
    ...
    var remakeGame = function remakeGame(item) {
        makeCards(item);
        resetScore();
    };
    ...
    var setActive = function setActive() {
        document.querySelectorAll(gameSelectors.cards).forEach(function (card) {
    ...
    var changeCards = function changeCards(state) {
        return new Promise(function (resolve, reject) {
            playSound();
            if (state.status === true) {

                document.querySelectorAll('.face').forEach(function (card, arr) {
                    if (card.dataset.card == state.code) {
                        setTimeout(function () {
                            var classNameList = card.parentElement.classList;
                ...
                document.querySelectorAll(gameSelectors.face).forEach(function (card) {
                    setTimeout(function () {
                        var classNameList = card.parentElement.classList;
    ...
    var reuiScore = function reuiScore(score) {
    ...
    var addFlipp = function addFlipp(trg) {
        trg.parentElement.classList.add('flipped');
    };

    var removeFlipped = function removeFlipped() {
        document.querySelectorAll(gameSelectors.flipped).forEach(function (card) {
    ...
    return {
        ...
        addFlipp: addFlipp,
        removeFlipped: removeFlipped
    };
}();

var App = function (model, ui) {

    var listenStartGame = function listenStartGame() {
    ...
    var playGame = function playGame() {
        var cardArr = model.calcCard();
        ui.startGame(cardArr);
        ui.setActive();
        ui.rotateCards();
    };

    var listenCardClcik = function listenCardClcik() {
        document.querySelector(ui.gameSelectors.cardsContainer).addEventListener('click', function (e) {
            ...
            if (trg.classList.contains('short')) {
                ...
                if (trg.parentElement.classList.contains('active')) {
                    ...
                    var state = model.checkPairs();

                    ui.changeCards(state).then(function (res) {
                        var points = model.checkCards(res.cardsCount, res.status);
    ...
    var playGameAgain = function playGameAgain() {
